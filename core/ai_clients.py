...
async def async_generate_text(provider_key, messages, system_prompt="", max_tokens=None, model_name=""):
    api_key = provider_key.get_api_key()
    model = model_name or provider_key.model_name
    if provider_key.provider == AIProviderKey.Provider.OPENAI:
        return await async_generate_openai_text(model, api_key, messages, system_prompt, max_tokens)
    if provider_key.provider == AIProviderKey.Provider.ANTHROPIC:
        return await async_generate_anthropic_text(model, api_key, messages, system_prompt, max_tokens)
    if provider_key.provider == AIProviderKey.Provider.GEMINI:
        return await async_generate_gemini_text(model, api_key, messages, system_prompt, max_tokens)
    raise AIProviderError("Unsupported AI provider.")
...
async def async_stream_text(provider_key, messages, system_prompt="", max_tokens=None, model_name=""):
    api_key = provider_key.get_api_key()
    model = model_name or provider_key.model_name
    if provider_key.provider == AIProviderKey.Provider.OPENAI:
        async for chunk in async_stream_openai_text(model, api_key, messages, system_prompt, max_tokens):
            yield chunk
        return
    if provider_key.provider == AIProviderKey.Provider.ANTHROPIC:
        async for chunk in async_stream_anthropic_text(model, api_key, messages, system_prompt, max_tokens):
            yield chunk
        return
    if provider_key.provider == AIProviderKey.Provider.GEMINI:
        async for chunk in async_stream_gemini_text(model, api_key, messages, system_prompt, max_tokens):
            yield chunk
        return
    raise AIProviderError("Unsupported AI provider.")
...
async def async_generate_openai_text(model, api_key, messages, system_prompt, max_tokens):
    try:
...
async def async_stream_openai_text(model, api_key, messages, system_prompt, max_tokens):
    try:
        from openai import AsyncOpenAI, OpenAIError
    except ImportError as exc:
        raise AIProviderError("The OpenAI SDK is not installed.") from exc
...
async def async_generate_anthropic_text(model, api_key, messages, system_prompt, max_tokens):
    try:
...
async def async_stream_anthropic_text(model, api_key, messages, system_prompt, max_tokens):
    try:
        from anthropic import AsyncAnthropic, AnthropicError
    except ImportError as exc:
        raise AIProviderError("The Anthropic SDK is not installed.") from exc
...
async def async_generate_gemini_text(model, api_key, messages, system_prompt, max_tokens):
    try:
        from google import genai
    except ImportError as exc:
        raise AIProviderError("The Google Gen AI SDK is not installed.") from exc
...
async def async_stream_gemini_text(model, api_key, messages, system_prompt, max_tokens):
    try:
        from google import genai
    except ImportError as exc:
        raise AIProviderError("The Google Gen AI SDK is not installed.") from exc
